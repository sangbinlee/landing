pipeline {
    ...
    stages {
        stage('Clone Sources') {
          steps {
            echo 'landing building the application...'
            // git 'https://gitlab.com/chiminyau/ci-test.git'
          }
        }
        stage('Information') {
          steps {
            echo 'landing Information...'
            sh 'node -v'
            sh 'npm -v'
          }
        }
        ...
        stage('Build') {
          steps {
            echo 'landing building the application...  now....'
            sh 'npm run build'
          }
        }
        // stage('build') {
        ...
        stage('test') {
            steps {
                echo 'landing testing the application...'
            }
        }
        stage('deploy') {
            steps {
                ...
                // sh 'pm2 restart ecosystem.config.cjs || pm2 start ecosystem.config.cjs'// [error] Missing `NUXT_UI_PRO_LICENSE` license key.
                ...
                // sh ' pm2 restart "landing" || pm2 start npm run preview --name "landing"'// [error] Missing `NUXT_UI_PRO_LICENSE` license key.
            }
        }
    }
}
